...
export default async function ConnectorMappingsPage({
  ...
  if (
    !profile ||
    !['platform_super_admin', 'mysoft_support_admin'].includes(profile.role)
  ) {
    redirect('/dashboard');
  }
  ...
  const { data: templates } = await admin
    .from('field_mappings')
    .select(
      'id, name, description, transaction_type, template_status, column_mappings, created_at, updated_at'
    )
    .eq('is_template', true)
    .eq('connector_id', id)
  ...
  for (const ot of objectTypes) {
    typeLabels[ot.key] = ot.displayName;
  }

  const totalPublished = (templates ?? []).filter(
    (t) => t.template_status === 'published'
  ).length;
  const totalDraft = (templates ?? []).filter(
    (t) => t.template_status === 'draft'
  ).length;
      ...
      {!(templates ?? []).length ? (
        ...
        >
          <p style={{ fontSize: 14, color: 'var(--muted)', margin: '0 0 16px' }}>
            No templates yet for this connector.
          </p>
          {canEdit && (
            <Link
              href={`/platform/connectors/${id}/mappings/new`}
                      ...
                      <tbody>
                        {group.map((t) => (
                          <tr key={t.id}>
                            ...
                            <td style={tdStyle}>
                              <span style={{ fontSize: 12, color: 'var(--muted)' }}>
                                {Array.isArray(t.column_mappings)
                                  ? t.column_mappings.length
                                  : 0}{' '}
                                field
                                {Array.isArray(t.column_mappings) &&
                                t.column_mappings.length !== 1
                                  ? 's'
                                  : ''}
                              </span>
                            </td>
                              ...
                              <td style={{ ...tdStyle, textAlign: 'right' }}>
                                ...
                                >
                                  ...
                                  <TemplateStatusButton
                                    templateId={t.id}
                                    currentStatus={t.template_status}
                                  />
                                </div>
                              </td>
                            )}
