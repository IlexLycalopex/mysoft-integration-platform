...
export async function POST(
  ...
  if (body.immediate) {
    try {
      ...
      return NextResponse.json({ ok: true, queued: true, executed: true, ...result });
    } catch (err: unknown) {
      const message = err instanceof Error ? err.message : String(err);
      return NextResponse.json({ ok: false, queued: true, executed: false, error: message });
    }
  }
